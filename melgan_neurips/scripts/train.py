...
def main():
    ...
    train_set = AudioDataset(
        Path(args.data_path) / "train_files.txt", args.seq_len, sampling_rate=48000
    )
    ...
    for i, x_t in enumerate(test_loader):
        ...
        save_sample(root / ("original_%d.wav" % i), 48000, audio)
        writer.add_audio("original/sample_%d.wav" % i, audio, 0, sample_rate=48000)
            ...
            if steps % args.save_interval == 0:
                ...
                with torch.no_grad():
                    for i, (voc, _) in enumerate(zip(test_voc, test_audio)):
                        ...
                        save_sample(root / ("generated_%d.wav" % i), 48000, pred_audio)
                        writer.add_audio(
